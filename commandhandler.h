 ...
 * Wrapper for exec(char** args). Runs the callback void() function in
 * *callback.
 *
 * Parameters:
 * char **args -> The array of strings that represent the command to be run.
 ...
 * Redirects the STDOUT of the command stored in **args (and in **parts[0]) to
 * the file specified in **parts[1]
 *
 * Parameters:
 * char **[arts] -> An array which has the command to be run, followed by the
 * file where STDOUT is redirected to. char **args -> An array which will be
 * passed on to exec_callback
 */
 ...
 * Redirects the STDIN of the command stored in **args (and in **parts[0]) from
 * a file specified in **parts[1]
 *
 * Parameters:
 * char **[arts] -> An array which has the command to be run, followed by the
 * file where STDIN is redirected from. char **args -> An array which will be
 * passed on to exec_callback
 */
